...
public class ParserGenerate : SymbolsGen
{
	...
	public void Create(string infname) 
	{
		...
		{
			erh.Error(new CSToolsFatalException(28,"Could not open input file "+infname+" ("+ex.Message+")."));
			return;
		}
		...
		{
			erh.Error(new CSToolsFatalException(29,"Could not create output file "+infname+".cs ("+ex.Message+")."));
			return;
		}
		_Create();
		m_outFile.Close();
	}
	public override SourceLineInfo sourceLineInfo(int pos) // 4.5c
	{
		return new SourceLineInfo (m_lexer,pos);
	}
	...
	public void GetDefs(string fname) 
	{
		try 
		{
			...
			sr.Close();
			if (!gotit)
				erh.Error(new CSToolsFatalException(27,"%parser directive did not give a tokens file"));
		}
		catch (Exception e)
		{
			erh.Error(new CSToolsFatalException(27,fname+": "+e.Message));
		}
	}
	...
	internal void DoFirst() 
	{
		...
		bool donesome = true;
		while (donesome) // code corrected by Wayne Kelly
		{
			donesome = false;
			foreach (Production p in prods)
				foreach (CSymbol t in p.m_rhs)
				{
	...
	public override void CopySegment() 
	{
		...
		for (;(ch=m_lexer.GetChar())!=0;) 
		{
			...
			if (ch=='\n') 
			{
				...
				m_outFile.WriteLine("#line "+m_inFile.lm.lines+" \""+m_inFile.fname+"\"");
				str ="";
			}
		}
		m_outFile.Write(str);
	...
	public override void ParserDirective() 
	{
		...
		m_actions = "public class yy"+m_outname+": YyParser {\n" +
	...
	public override void AssocType(Precedence.PrecType pt, int p) 
	{
		...
		while (p<line.Length) 
		{
			...
			bool rej = false;
			s = (CSymbol)((yyptokens)(m_lexer.tokens)).OldAction(m_lexer,ref m_lexer.yytext,action,ref rej);
			s = s.Resolve();
	...
	{  
		...
		if (ch<'0' || ch>'9')
			...
			ix = minus?('0'-ch):(ch-'0');
		for (ch = m_lexer.PeekChar(); ch>='0' && ch<='9'; ch = m_lexer.PeekChar() ) // 4.5f
		{
			ix = ix*10+ (minus?('0'-ch):(ch-'0'));
			m_lexer.Advance(); //4.5f
		}
		if (ix<=0) 
		{
	...
	{ // ch is the start bracket, already returned
		...
		while ((ch=m_lexer.GetChar())!=0) 
		{
			...
			else if ((ch>='a'&&ch<='z')||(ch>='A'&&ch<='Z')||ch=='_') 
			{
				...
				if (quoteseen==0 && m_prod.m_alias.Contains(a.yytext))  // 4.4b
				{
	...
	public static void Main(string[] argv) 
	{
		...
		for (j = 0; argc>0 && argv[j][0]=='-'; j++,argc--)
			switch(argv[j][1]) 
			{
				...
				case 'L': Lflag = false; break; // skip LALR phase
