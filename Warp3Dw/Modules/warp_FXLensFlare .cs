...
namespace Warp3Dw
{
	public class warp_FXLensFlare : warp_FXPlugin
	{
		...
		int[] getPalette (int[] color, int[] alpha)
		{
			int r, g, b;
		...
		warp_Texture createGlow (int w, int h, int color, int alpha)
		{
			return createRadialTexture (w, h, getGlowPalette (color), getConstantAlpha (alpha));
		}

		warp_Texture createRing (int size, int color)
		{
			return createRadialTexture (size, size, getColorPalette (color, color), getRingAlpha (40));
		}

		warp_Texture createSec (int size, int sizedelta, int color, int colordelta)
		{
		...
		int[] getConstantAlpha (int alpha)
		{
		...
		int[] getColorPalette (int color1, int color2)
		{
			int[] palette = new int[256];
